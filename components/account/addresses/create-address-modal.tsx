...
export default function CreateAddressModal({
  ...
  const handleFieldChange = <K extends keyof CreateAddressInput>(field: K, value: CreateAddressInput[K]) => {
    setForm((prev) => ({ ...prev, [field]: value }));
  };

  const handleSubmit = () => {
    const result = CreateAddressInputSchema.safeParse(form);
    if (!result.success) {
            ...
            <div className="flex flex-wrap items-center gap-2">
              {addressTypes.map(({ label, value }) => (
                <button
                  key={value}
                  type="button"
                  // Pass the actual integer value here
