...
This is the living visual specification for the KOA storefront and the visual
north star for adjacent account and admin work. It is a design direction, not a
replacement for the engineering rules in `AGENTS.md` or the currently implemented
component rules in the repository root `DESIGN.md`.

Read the root `DESIGN.md` before changing user-facing code. Where this guide
proposes a future storefront treatment that has not yet been implemented, the
root guide and the existing semantic tokens remain the compatibility contract
until a deliberate token migration is approved.
...
- The current repository maps `--primary` to a near-black operational value.
  Do not replace it ad hoc. Introduce or remap the action token through a
  deliberate theme/token change, and keep admin behavior stable during that
  migration.
...
- **Oswald** or another approved geometric condensed display face may be used
  for campaign/editorial display moments only. It must not become the default
  UI font.
...
If a second display font is not loaded, use Inter at a heavier display weight
rather than introducing a third visual language. Font loading is a deliberate
performance and branding decision, not a per-page embellishment.

### Type scale

| Role | Treatment | Use |
| --- | --- | --- |
| Campaign display | `64px`, `0.92`, `800`, uppercase, tight tracking | Hero statements and major campaign moments |
| Editorial display | `56px`, `1.05`, `500`, sentence case | Premium editorial statements and quieter brand moments |
| Section display | `40px`, `1.0`, `800`, uppercase, tight tracking | Storefront section openers |
...
- **Account and admin:** `16–24px` between groups; use the compact patterns in
  the root `DESIGN.md` for repeated operational work.
...
- Keep durable implemented styling decisions in the root `DESIGN.md`.
...
- Load Oswald, if approved, only for storefront campaign surfaces and measure the
  performance impact before adding it to the root layout.
- Update the root `DESIGN.md` in the same change when a new token, primitive,
...
:root {
  ...
  --koa-surface: var(--koa-primitive-surface);
...
- [ ] The root `DESIGN.md` was read before implementation.
...
- [ ] Any new reusable visual decision is documented in the root `DESIGN.md`
      when it becomes part of the implemented system.

## 17. Open decisions

The following decisions can evolve as the storefront is built and tested:

- Whether the action color should remain the proposed KOA blue or align with a
  future KOA brand palette.
- Whether an approved condensed display font is worth adding, or whether Inter
  display weights are sufficient.
...
- The final dark-mode contrast values for every semantic state.
- Which storefront routes are ready to move from current neutral tokens to the
  action-color and editorial surface direction.

Resolve these through small, documented token changes and user-facing
prototypes. Do not resolve them by creating a parallel component style.
